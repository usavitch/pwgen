; ======================================================================
; c24.asm - CSPRNG С 3D-ДИФФУЗИЕЙ МЕЖДУ СТАКАНЧИКАМИ
; ======================================================================
...
; ======================================================================
; 3D ДИФФУЗИЯ: XOR результата в соседний стаканчик
; Вход: r8d = текущий индекс стаканчика
;       eax, ebx, ecx, edx содержат результат CUP_QR
; ======================================================================
...
; ======================================================================
; ИНИЦИАЛИЗАЦИЯ КУБА (с 3D-диффузией)
; ======================================================================
init_cups:
    PUSH_CALLEE
    mov     ecx, CUPS_SIZE
...
.balls_ok:
    ...
    shl     rax, 6
    lea     rdi, [cups + rax]
    mov     r8d, r13d              ; сохраняем индекс для INTER_CUP_DIFFUSION
    xor     r15d, r15d
.fill_balls:
    cmp     r15d, r14d
    jae     .next_cup
...
; ======================================================================
; РАЗБРАСЫВАНИЕ ШАРИКОВ (с 3D-диффузией)
; ======================================================================
collect_and_splash:
    PUSH_CALLEE
    mov     r12d, 10
.splash_loop:
    ...
    mov     r13d, eax                    ; текущий индекс стаканчика
    mov     r8d, r13d                    ; сохраняем для INTER_CUP_DIFFUSION
    mov     eax, r13d
    shl     rax, 6
    lea     rdi, [cups + rax]
    push    rdi
...
.write_ball:
    mov     r9d, eax                     ; сохраняем ball index
    lea     rsi, [temp_ball]
    CUP_QR
    ; 3D-ДИФФУЗИЯ: пробрасываем результат в соседний стаканчик
    push    rdi
    push    rax
    push    rbx
    push    rcx
    ...
    pop     rax
    pop     rdi
    ; Завершаем запись в текущий стаканчик
    mov     ebx, [temp_ball]
    mov     [rdi + r9*4], ebx
...
; ======================================================================
; ИЗВЛЕЧЕНИЕ ШАРИКОВ (с 3D-диффузией)
; ======================================================================
sip_from_cups:
    PUSH_CALLEE
    mov     r12d, 8
...
.sip_loop:
    call    get_random_position
    mov     r14d, eax
    mov     r8d, r14d                    ; сохраняем для INTER_CUP_DIFFUSION
...
; ======================================================================
; ОЧИСТКА ПАМЯТИ (усиленная)
; ======================================================================
burn_memory:
    PUSH_CALLEE
    ; 1. Затираем все буферы случайными данными (через XOR с собой)
    lea     rdi, [state]
    mov     ecx, 48
    mov     eax, 0xDEADBEEF
    rep stosd
    ...
    xor     eax, eax
    rep stosd
    ; 3. Затираем счётчики и буфер пароля
    lea     rdi, [cup_ball_count]
    mov     ecx, CUPS_X * CUPS_Y
    rep stosb
    lea     rdi, [pass]
    mov     ecx, 64
    rep stosb
    ; 4. Обнуляем служебные переменные
    mov     dword [pool_pos], 0
    mov     dword [cups_fill_count], 0
    mov     dword [cups_take_count], 0
    mov     dword [temp_ball], 0
    ; 5. Зачистка XMM регистров
    pxor    xmm0,  xmm0
    pxor    xmm1,  xmm1
    pxor    xmm2,  xmm2
    pxor    xmm3,  xmm3
